    ...
    return (
        ...
        <div>
            < div className="grid lg:grid-cols-2 gap-4" >
                <button onClick={() => handlePaymentTypeChange(false)} className={`${isCardVisible ? 'text-gray-500 border-2' : ' bg-primary text-white'}  text-center rounded py-2 font-semibold `}>{option1}</button>
                <button onClick={() => handlePaymentTypeChange(true)} className={`${isCardVisible ? 'text-white border-2 bg-primary' : ' text-gray-500 border-2'}  text-center rounded py-2 font-semibold `}>{option2}</button>
            </div >
            {
                isCardVisible ? (
